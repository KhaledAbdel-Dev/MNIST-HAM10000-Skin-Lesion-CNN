...
def main():
    ...
    rows = load_metadata(str(meta_csv))
    rows = [r for r in rows if r["dx"] in CLASS_MAP]  # filter to 7 classes
    ...
    class_counts = np.bincount(labels_train, minlength=len(CLASS_MAP))
    ...
    # Outputs
    run_name = args.run_name or f"{args.backbone}_img{args.img_size}_seed{args.seed}"
